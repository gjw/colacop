...
- Stack locked in `ARCHITECTURE.md`: Node 24 / TypeScript, React+Vite, Express, Zod, Postgres in Docker + Kysely, Chokidar watcher, PM2/nginx VPS. Async ingestion → worker → results → review UI. Model/OCR provider behind a small interface.
...
- **Deliverables are dual:** source repo + a *live deployed URL*. Don't defer deployment to the end.
- **Reviewer-friendliness over ambition.** Working core with clean code beats incomplete-but-clever per the assignment.
- **Provenance discipline.** New requirements should trace back to facts; new facts cite a source.
