...
Include the issue ID in commit messages: `Fix auth validation (cc-a1b2)`

### Discovering Work

When you find something that needs doing while working on a task, create an issue
and move on — don't go fix it:

```bash
br create "Found: missing validation" -t bug -p 1 -d "Details"
br label add <id> bug
```

### Standard Labels

Apply these when creating or triaging beads. Milestone labels (e.g., `mvp`,
`post-mvp`) are project-specific and defined in CONTEXT.md.

| Label | Meaning |
|-------|---------|
| `bug` | Broken behavior |
| `chore` | Maintenance, refactoring, no user-facing change |
| `reevaluate` | Questionable value — don't start without Chair confirmation |
...
1. **Present for review** — summarize what you built, report test results, note
   any deviations from the plan. Wait for Chair's feedback.
2. **Iterate on feedback** — address issues, re-run tests, present again.
   Repeat until Chair approves.

Do not proceed to Finishing Steps until Chair explicitly approves.

## Finishing Steps

After Chair approves your work, complete the following before closing.

### Core Sequence

1. **Definition of Done** — verify all acceptance criteria from the beads issue
   are met. Run the full test suite one final time.
2. **Change doc** — if CONTEXT.md specifies a change doc format, produce one for
   this task. Place it in the location CONTEXT.md designates.
3. **Doc hygiene** — check each, update if applicable:

   - [ ] New decision? → decisions doc
   - [ ] New term? → glossary
   - [ ] Invariant changed? → invariants doc
   - [ ] Question resolved? → open-questions doc

   File paths are defined in CONTEXT.md. If a doc doesn't exist yet and the
   checklist applies, create it.

4. **Friction review** — one or two sentences: what slowed you down, what was
   unclear, what should change. Chair uses these to improve the system.

### Close & Commit

Do these **in this exact order** — the commit must capture the closed bead state:
...
## Wrap Up (Mid-Task Interruption)

If Chair says "wrap up" before work is complete:

1. **Summarize current state** — what's done, what's in progress, what's blocked.
2. **Friction review** — brief, same as finishing sequence.
3. **Persist state to the bead:**

   ```bash
   br update <id> --notes "WIP: <what's done, what remains, any gotchas>"
   ```

4. **Commit WIP to branch:**

   ```bash
   br sync --flush-only
   git add -A
   git commit -m "WIP: <what's done> (<issue-id>)"
   ```

5. **Signal:** `CHECKPOINT: <what's done so far, what's next>`

Do NOT close the bead. The next agent picks up from your notes.
